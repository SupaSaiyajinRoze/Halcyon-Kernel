...
int g_temp_status = TEMP_POS_10_TO_POS_30;
...
extern CHR_CURRENT_ENUM  usb_charging_current;
extern int mhl_connect;
 /* ============================================================ // */
 /* extern function */
 /* ============================================================ // */
#ifdef CONFIG_CHARGER_BQ2589X
extern int bq25890_get_vbus_type_external(void);
extern int bq2589x_auto_dpdm_detect(int mtk_charger_type, int enable);
#endif
 /* ============================================================ // */
...
if (g_temp_status == TEMP_ABOVE_POS_50)//55~
	{
		cv_voltage = BATTERY_VOLT_04_100000_V; //4//4.1v
	} 
	else if (g_temp_status == TEMP_POS_45_TO_POS_50) //45~55
	{
		cv_voltage = BATTERY_VOLT_04_400000_V;//4//4.4v
	}
	else if (g_temp_status == TEMP_POS_15_TO_POS_45)//15~45
	{
		cv_voltage = BATTERY_VOLT_04_400000_V;//4//4.4v
	}
	else if (g_temp_status == TEMP_POS_0_TO_POS_15) //0~15
	{
		cv_voltage = BATTERY_VOLT_04_400000_V; //4//4.1v
	} 
	else //~0
	{
		cv_voltage = BATTERY_VOLT_04_100000_V;//4//4.1v
	...
	if (g_temp_status == TEMP_ABOVE_POS_60)//55~
	{
		cv_voltage = JEITA_TEMP_ABOVE_POS_60_CV_VOLTAGE; //4.1v
	} 
	else if (g_temp_status == TEMP_POS_45_TO_POS_60) //45~55
	{
		cv_voltage = JEITA_TEMP_POS_45_TO_POS_60_CV_VOLTAGE;//4.1
	}
	else if (g_temp_status == TEMP_POS_30_TO_POS_45)//30~45
	{
		cv_voltage = JEITA_TEMP_POS_10_TO_POS_45_CV_VOLTAGE;//4.34
	}
	else if (g_temp_status == TEMP_POS_10_TO_POS_30) //10~30
	{
		cv_voltage = JEITA_TEMP_POS_0_TO_POS_10_CV_VOLTAGE; //4.34
	} 
...
static void set_jeta_max_charge_current(void)
{
#ifdef CONFIG_USB_IF
...
static void set_jeta_mhl_charge_current(void)
{
#ifdef CONFIG_USB_IF
	if (BMT_status.charger_type == STANDARD_HOST)
		return;
	...
	battery_xlog_printk(BAT_LOG_CRTI, "%s[BATTERY] JEITA set charging current : %d, %d\r\n",
			__func__, g_temp_CC_value, g_temp_input_CC_value);
...
static void set_jeita_charging_host_current(void)
{
#ifdef CONFIG_USB_IF
	if (BMT_status.charger_type == STANDARD_HOST)
		return;
...
static void set_jeta_standard_host_current(void)
{
#ifdef CONFIG_USB_IF
	if (BMT_status.charger_type == STANDARD_HOST)
		return;
...
PMU_STATUS do_jeita_state_machine(void)
{
	...
	if (BMT_status.temperature >= TEMP_POS_60_THRESHOLD) {
		...
		g_temp_status = TEMP_ABOVE_POS_50;

		return PMU_STATUS_FAIL;
	} 
	else if (BMT_status.temperature >= TEMP_POS_45_THRESHOLD)	//45-50
	{
		if ((g_temp_status == TEMP_ABOVE_POS_50) && (BMT_status.temperature >= TEMP_POS_60_THRES_MINUS_X_DEGREE))
		{
			battery_log(BAT_LOG_CRTI,
			...
			g_temp_status = TEMP_POS_45_TO_POS_50;
		}
	} 
	else if (BMT_status.temperature >= TEMP_POS_10_THRESHOLD) //15-45
	{
		if (((g_temp_status == TEMP_POS_45_TO_POS_50) && (BMT_status.temperature >= TEMP_POS_30_THRES_MINUS_X_DEGREE))
			|| ((g_temp_status == TEMP_POS_0_TO_POS_15) && (BMT_status.temperature <= TEMP_POS_10_THRES_PLUS_X_DEGREE)))
		{
			battery_log(BAT_LOG_CRTI,
	...
	else if (BMT_status.temperature >= TEMP_POS_0_THRESHOLD) //0-15
	{
	...
	if (BMT_status.temperature >= TEMP_POS_60_THRESHOLD) //55~
	{
		battery_log(BAT_LOG_CRTI,
				    "[BATTERY] Battery Over high Temperature(%d) !!\n\r",
				    TEMP_POS_60_THRESHOLD);
	...
	else if (BMT_status.temperature >= TEMP_POS_30_THRESHOLD) //30-45
	{
		if (((g_temp_status == TEMP_POS_45_TO_POS_60) && (BMT_status.temperature >= TEMP_POS_30_THRES_MINUS_X_DEGREE))
			|| ((g_temp_status == TEMP_POS_10_TO_POS_30) && (BMT_status.temperature <= TEMP_POS_30_THRES_PLUS_X_DEGREE)))
		{
			battery_log(BAT_LOG_CRTI,
					    "[BATTERY] Battery Temperature not recovery to normal temperature charging mode yet!!\n\r");
		} 
		...
		{
			g_temp_status = TEMP_POS_30_TO_POS_45;
		}
	}
	else if (BMT_status.temperature >= TEMP_POS_10_THRESHOLD) //10-30
	{
		if (((g_temp_status == TEMP_POS_30_TO_POS_45) && (BMT_status.temperature >= TEMP_POS_45_THRES_MINUS_X_DEGREE))
		...
		else 
		{
			...
			g_temp_status = TEMP_POS_10_TO_POS_30;
		}
	} 
	else if (BMT_status.temperature >= TEMP_POS_0_THRESHOLD) //0-10
	{
		if ((g_temp_status == TEMP_BELOW_NEG_10) && (BMT_status.temperature <= TEMP_POS_0_THRES_PLUS_X_DEGREE))
		{
			battery_log(BAT_LOG_CRTI,
						    "[BATTERY] Battery Temperature between %d and %d,not allow charging yet!!\n\r",
...
void select_charging_curret(void)
{
	...
	if (g_ftm_battery_flag) {
        ...
        #ifdef CONFIG_CHARGER_BQ2589X
		bq_charge_type = bq25890_get_vbus_type_external();
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] bq_charge_type= %d, BMT_status.charger_type=%d\n", bq_charge_type, BMT_status.charger_type);
		bq2589x_auto_dpdm_detect(BMT_status.charger_type, true);
        #endif
        
#if defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
        if(BMT_status.charger_type == STANDARD_HOST)
            set_jeta_standard_host_current();
        #ifdef CONFIG_CHARGER_BQ2589X
		else if (bq_charge_type == BQ2589X_VBUS_MAXC)
			set_jeta_max_charge_current();
        else if(mhl_connect)
            set_jeta_mhl_charge_current();
        #endif
		else if ((BMT_status.charger_type == STANDARD_CHARGER) || (BMT_status.charger_type == NONSTANDARD_CHARGER))
		    set_jeita_charging_current();
        else if(BMT_status.charger_type == CHARGING_HOST)
            set_jeita_charging_host_current();
#endif
	}
...
#if defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
#ifdef CONFIG_CHARGER_BQ2589X
	if ((g_temp_status == TEMP_ABOVE_POS_50) || (g_temp_status == TEMP_BELOW_0)) {
		temp_error_recovery_chr_flag = KAL_FALSE;
	}
	if ((temp_error_recovery_chr_flag == KAL_FALSE) && (g_temp_status != TEMP_ABOVE_POS_50)
	    && (g_temp_status != TEMP_BELOW_0)) {
